...
# # 9- Repeat set 1, 2, 3, 4, 5, 6 on “Daily-total-female-births.csv” dataset. You will be predicting the Births for this dataset.
# 
# ## 1- Load the Sales values of dataset “Daily-total-female-births.csv”. The purpose is to perform one-step ahead prediction on Sales value.

# In[2]:


Dailybirths=pd.read_csv('Daily-total-female-births.csv')
Dailybirths.head()
Dailybirths.info()
...
Yhatnaive=[]
for i in range(3,366):
    Yhatnaive.append(Dailybirths.iloc[i-2,1])
len(Yhatnaive)
...
Yhatdrift=[]
for i in range(3,366): 
    Yhatdrift.append((Dailybirths.iloc[i-2,1]+(h*((Dailybirths.iloc[i-2,1]-Dailybirths.iloc[0,1])/(i-2)))))
...
plt.plot(obstrue,Dailybirths.iloc[:,1],label='True values',marker="o",markersize=15,color="green")
...
plt.plot(obstrue,Dailybirths.iloc[:,1],label='True value',marker="o",markersize=20,color="green")
...
resmean=Dailybirths.iloc[2:,1]-Yhatmean
...
resnaive=Dailybirths.iloc[2:,1]-Yhatnaive
...
resdrift=Dailybirths.iloc[2:,1]-Yhatdrift
