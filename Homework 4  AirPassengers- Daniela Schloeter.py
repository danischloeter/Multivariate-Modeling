...
# # 8- Repeat set 1, 2, 3, 4, 5, 6 on “AirPassenegers.csv” dataset. You will be predicting the # of Passengers for this dataset.
# 
# ## 1- Load the Sales values of dataset “AirPassenegers.csv”. The purpose is to perform one-step ahead prediction on Sales value.
...
for i in range(3,145):
    yhatmean=AirPassengers.iloc[:i-1,1]
...
Yhatnaive=[]
for i in range(3,145):
    Yhatnaive.append(AirPassengers.iloc[i-2,1])
len(Yhatnaive)
...
Yhatdrift=[]
for i in range(3,145): 
    Yhatdrift.append((AirPassengers.iloc[i-2,1]+(h*((AirPassengers.iloc[i-2,1]-AirPassengers.iloc[0,1])/(i-2)))))
...
plt.plot(obstrue,AirPassengers.iloc[:,1],label='True values',marker="o",markersize=15,color="green")
...
plt.plot(obspred,Yhatnaive,label='Predicted values naive method',marker="*",markersize=20,color="purple")
plt.plot(obstrue,AirPassengers.iloc[:,1],label='True value',marker="o",markersize=20,color="green")
...
plt.plot(obspred,Yhatdrift,label='Predicted value drift method',marker="*",markersize=40,color="purple")
plt.plot(obstrue,AirPassengers.iloc[:,1],label='True value',marker="o",markersize=20,color="green")
...
resmean=AirPassengers.iloc[2:,1]-Yhatmean
...
resnaive=AirPassengers.iloc[2:,1]-Yhatnaive
...
resdrift=AirPassengers.iloc[2:,1]-Yhatdrift
